...
export async function shiftsRoutes(app: FastifyInstance) {
  ...
  app.post("/:id/start", { preHandler: requireActor("PILOT") }, async (request, reply) => {
    const { id } = request.params as { id: string };
    return reply.send(await shiftsService.startShift(id, request.user.sub));
  });

  app.post("/:id/end", { preHandler: requireActor("PILOT") }, async (request, reply) => {
    const { id } = request.params as { id: string };
    return reply.send(await shiftsService.endShift(id, request.user.sub));
  });

  app.post("/:id/breaks", { preHandler: requireActor("PILOT") }, async (request, reply) => {
  ...
  app.post("/:id/breaks/:breakId/end", { preHandler: requireActor("PILOT") }, async (request, reply) => {
